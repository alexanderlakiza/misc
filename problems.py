...
def read_csvs():
    agrs = pd.read_csv('agregates.csv')
    links = pd.read_csv('agregates_warehouses.csv')
    whouses = pd.read_csv('warehouses.csv')
...
def get_problem(agrs, links, whouses, agregate, date=None):
    ...
    if len(agrs[(agrs['Name'] == agregate) & (agrs['dStart'] == date)]) == 0:
    ...
    key_agr_ids = list(links['Идентификатор'].unique())  # Агрегаты, для которых у нас есть склады

    if date:
        counter = list(agrs[(agrs['Name'] == agregate) &
                            (agrs['dStart'] == date)]['OccupiedPercentage'] == 100).count(False)
    else:
        counter = list(agrs[agrs['Name'] == agregate]['OccupiedPercentage'] == 100).count(False)
    ...
    outputs = list(links[links['Идентификатор'] == agr_id]['OutputSP'].dropna())
    ...
    if date:
        for sklad in input_whouses:
            current_sklad = whouses[(whouses['Идентификатор'] == sklad) & (whouses['Плановая дата'] == date)]
...
def main():
    ...
    for name in names:
        print(get_problem(agrs, links, whouses, name, '2021-10-07'))
